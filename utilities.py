...
def extract_features_2d(df, lookback=10):
    ...
    for i in range(len(df)):
        ...
        data.append(
            np.array(data_i, dtype=np.float32).reshape(
                lookback, len(cols_of_interest), 1
            )
        )
...
def extract_features_1d(df, lookback=10):
    ...
    for i in range(len(df)):
        ...
        data.append(
            np.array(data_i, dtype=np.float64).reshape(
                1, len(cols_of_interest) * lookback
            )
        )
        data[i] = data[i][0]
...
def extract_features_cols(df, lookback=10):
    ...
    out_df.loc[:, "time"] = time
    # add price col
    price = []
    for i in range(len(df)):
        price.append(df.iloc[i]["price"])
    out_df.loc[:, "price"] = price
    # add class col
    class_arr = []
    for i in range(len(df)):
        class_arr.append(df.iloc[i]["class"])
    out_df.loc[:, "class"] = class_arr
    # add class col
    diff = []
    for i in range(len(df)):
        diff.append(df.iloc[i]["diff"])
    out_df.loc[:, "diff"] = diff
    return out_df
