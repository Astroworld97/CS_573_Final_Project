...
def run_neural_net(TICKER, LOOKBACK, LOOKAHEAD, LABEL_COL='close'):
    ...
    dense_model.fit(x=x_train, y=y_train, validation_split=0.3, shuffle=True, batch_size=8, epochs=25)
    ...
    plot_predictions(preds, y_test, save=False, name='regression'+str(LOOKBACK)+'_'+str(LOOKAHEAD)+'.png')
