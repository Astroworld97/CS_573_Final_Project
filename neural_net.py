...
def run_neural_net_regression(TICKER, LOOKBACK, LOOKAHEAD, LABEL_COL='close'):
    ...
    dense_model.fit(x=x_train, y=y_train, validation_split=0.3, shuffle=True, batch_size=16, epochs=250)
...
def run_neural_net_regression_diff(TICKER, LOOKBACK, LOOKAHEAD, LABEL_COL='close'):
    ...
    #fit model
    dense_model.compile(optimizer='adam', loss='mean_squared_error', metrics='mean_absolute_error')
    dense_model.fit(x=x_train, y=y_train, validation_split=0.3, shuffle=True, batch_size=64, epochs=50)
